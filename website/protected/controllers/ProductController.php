...
class ProductController extends AppController
{
    public function actionIndex(){
        ...
        $id = $_GET['id'];
//        $this->data['PRODUCTSALL'] = $this->getProductAll();
        ...
        $criteria->addCondition('infotype="product"','AND');
        $criteria->join = " LEFT JOIN xm_module module ON module.module_id=t.module_id  LEFT JOIN xm_article_product product ON product.article_id=t.article_id ";
    ...
    public function actionView($id){
        ...
        //上一个商品
        $prevproduct = Yii::app()->cache->get("prevproduct_".$article['article_id']);
        if($prevproduct==false){
            $prevproduct = $this->getPrevArticle($article['article_id'],$article['module_id']);
            Yii::app()->cache->set("prevproduct_".$article['article_id'], $prevproduct);
        }
        $this->data['prevproduct'] = $prevproduct;

        //下一篇商品
        $nextproduct = Yii::app()->cache->get("nextproduct_".$article['article_id']);
        if($nextproduct==false){
            $nextproduct = $this->getNextArticle($article['article_id'],$article['module_id']);
            Yii::app()->cache->set("nextproduct_".$article['article_id'], $nextproduct);
        }
