    ...
    <div class="box">
        ...
        <div class="form" >
            <form name="form_edit" id="form_edit" action="edit" method="post" >
                <input type="hidden" name="download_id" value="<?php echo $id; ?>" />
                ...
                <table class="form">
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="small validate[required] " name="title" value="<?php echo $download['title']; ?>" />&nbsp;&nbsp;<span class="red">*</span></td>
                    </tr>
                    <tr>
                        <td class="label">文件</td>
                        <td><input type="text" class="small" id="fileurl" name="fileurl" value="<?php echo $download['fileurl']; ?>" />
                            <input type="hidden" name="filetitle" id="filetitle" value="<?php echo $download['filetitle']; ?>"/>
                            &nbsp;&nbsp;<button class="btn" id="insertfile">选择文件</button></a>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php
                                ...
                                ?>
                                <input type="text" class="small validate[required,custom[integer]] " name="orderby" value="<?php echo $download['orderby'] ?>"  />
                            <?php
                            }
                            ?>
                        </td>
...
<script>
    var module_setting = {
        ...
        async: {
            enable: true,
            url: "../module/treeByType?type=download&moduleid=<?php echo $download['module_id']; ?>"
        },
        check:{
            chkStyle: "checkbox",
            enable:true,
    ...
    KindEditor.ready(function(K) {
        ...
        var fileupload = K.editor({
            allowFileManager : true,
            uploadJson:'<?php echo Yii::app()->request->baseUrl?>/extra/kindeditor/upload_json.php',
            fileManagerJson:'<?php echo Yii::app()->request->baseUrl?>/extra/kindeditor/file_manager_json.php'
        });
        K('#insertfile').click(function() {
            fileupload.loadPlugin('insertfile', function() {
                fileupload.plugin.fileDialog({
                    fileUrl : K('#fileurl').val(),
                    clickFn : function(url, title) {
                        debugger;
                        K('#fileurl').val(url);
                        $("input[name=filetitle]").val(title);
                        fileupload.hideDialog();
                    }
                });
            });
        });
